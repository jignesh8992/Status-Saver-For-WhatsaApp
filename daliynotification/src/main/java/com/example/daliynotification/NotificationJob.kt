...
class NotificationJob {
    ...
    fun setHour(@IntRange(from = 1, to = 24) hour: Int): NotificationJob {
        return mNotificationJob!!.apply {
            this.mHour = hour
        }
    }

    fun setMinute(minute: Int): NotificationJob {
        return mNotificationJob!!.apply {
            this.mMinute = minute
        }
    }
    ...
    private fun createJob(context: Context): Job? {
        ...
        Log.d(
            "TAGS",
            "Interval : $mIntervalDay - $mHour - $mMinute == ${mSharedPreferences.getString(
                "unique_tags",
                "NotificationTestTag"
            )!!} == ${mHour >= currentDate.get(Calendar.HOUR_OF_DAY) && mMinute >= currentDate.get(Calendar.MINUTE)}"
        )
